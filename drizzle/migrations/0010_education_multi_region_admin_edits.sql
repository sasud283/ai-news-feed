-- Education taxonomy, bilateral geography and protected post-publication editing.
ALTER TYPE public.story_topic ADD VALUE IF NOT EXISTS 'Education';

ALTER TABLE public.story_tags
  ADD COLUMN secondary_geography public.story_geography,
  ADD CONSTRAINT story_tags_distinct_geographies
    CHECK (secondary_geography IS NULL OR secondary_geography <> geography),
  ADD CONSTRAINT story_tags_worldwide_is_exclusive
    CHECK (secondary_geography IS NULL OR (geography <> 'Worldwide' AND secondary_geography <> 'Worldwide'));
...
CREATE FUNCTION public.update_published_story(p_story_id uuid, p_correction jsonb)
RETURNS void LANGUAGE plpgsql SECURITY INVOKER SET search_path = public AS $$
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN
    RAISE EXCEPTION 'Administrator access required' USING ERRCODE = '42501';
...
REVOKE ALL ON FUNCTION public.update_published_story(uuid, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.update_published_story(uuid, jsonb) TO authenticated;

CREATE FUNCTION public.queue_published_story_for_edit(p_story_id uuid)
RETURNS void LANGUAGE plpgsql SECURITY INVOKER SET search_path = public AS $$
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin') THEN
    RAISE EXCEPTION 'Administrator access required' USING ERRCODE = '42501';
  ...
  IF NOT EXISTS (
    ...
    WHERE id = p_story_id AND publication_status = 'published'
  ) THEN
    RAISE EXCEPTION 'Published story not found';
  END IF;
  INSERT INTO public.spot_check_queue(story_id, reason)
    SELECT p_story_id, 'admin_edit'
    WHERE NOT EXISTS (
      SELECT 1 FROM public.spot_check_queue
      WHERE story_id = p_story_id AND status = 'pending'
    );
END;
$$;

REVOKE ALL ON FUNCTION public.queue_published_story_for_edit(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.queue_published_story_for_edit(uuid) TO authenticated;
